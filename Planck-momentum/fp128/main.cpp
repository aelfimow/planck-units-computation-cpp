...
int main(int, char *[])
{
    ...
    fp128 const momentum = sqrtq((h_reduced * powq(c, 3.0Q)) / G);

    std::vector<char> buffer(256U);

    ::quadmath_snprintf(buffer.data(), buffer.size(), "%.36Qe", momentum);

    std::string const lp_str(buffer.data());

    std::cout << "Planck momentum (fp128): " << lp_str << std::endl;
